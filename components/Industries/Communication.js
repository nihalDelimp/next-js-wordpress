...
function Communication({ communicationData }) {
   return (
      <div>
               ...
               <div className="row pt-8 pb-4 pt-lg-1 pb-lg-1">
                  <div className="col-lg-12 text-left">
                     <ul className="alfanar-breadcrumb font-bold no-margin-bottom">
                        ...
                        <li className="alfanar-breadcrumb-item" aria-current="page"><a className="text-white" href="#">Industries : Communication</a></li>
                     </ul>
                  </div>
               </div>
               <div className="row">
                  <div className="pt-2 pb-2 col-lg-6 align-self-center wow">
                     <h1 className="text-white font-bold bottom-buffer-2point5rem">{communicationData[0]?.heading}</h1>
                     <p className="text-white font-medium extra-large">{communicationData[0]?.description}</p>
                  </div>
                  <div className="pt-2 pb-2 col-lg-6 align-self-center text-lg-right text-center wow">
                     <img className="w-100" src={communicationData[0]?.image?.sourceUrl} />
                  </div>
               </div>
            </div>
         </section>
               ...
               <div className="row pt-4 pb-4 pt-lg-5 pb-lg-5">
                  <div className="col-lg-6 my-auto wow slideInRight  pt-4 pb-4" data-wow-duration="2s" data-wow-delay="250ms">
                     <h1 className="text-primary font-bold  bottom-buffer-1point5rem">
                        {communicationData[1]?.heading}
                     </h1>
                     <p className="extra-large" style={{ color: "#808080" }}>{communicationData[1]?.description}</p>
                  </div>
                  <div className="col-lg-6 wow slideInLeft  pl-lg-8" data-wow-duration="2s" data-wow-delay="250ms">
                     {communicationData[1]?.listItems && communicationData[1]?.listItems.length > 0 &&
                        communicationData[1].listItems.map((item, index) =>
            ...
            <div id="industry-container" className="container-lg pb-4 pt-4 pb-lg-6 pt-lg-6">
               <div className="row">
                  <div className="col-lg-12 text-center pb-4 pt-4 wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.3s">
                     <h1 className="text-white font-bold  bottom-buffer-2point5rem">{communicationData[2]?.heading}</h1>
                     <p className="text-white extra-big font-medium ">{communicationData[2]?.description}</p>
                  </div>
               </div>
               <div className="row">
                  {communicationData[2].cardBox && communicationData[2].cardBox.length > 0 &&
                     communicationData[2].cardBox.map((item, index) =>
            ...
            <div id="top-clients-container" className="container-lg pb-4 pt-4">
               <div className="row pb-4 pt-4 wow pulse" data-wow-duration="2s">
                  <div className="col-12 text-center">
                     <h1 className="font-weight-bold">{communicationData[3]?.heading}</h1>
                  </div>
               </div>
                     ...
                     <div className="swiper-alfanar">
                        <div className="swiper-container top-clients-swiper">
                           <div className="swiper-wrapper">
                              {communicationData[3].carouselImages && communicationData[3].carouselImages.length > 0 &&
                                 communicationData[3].carouselImages.map((item, index) =>
